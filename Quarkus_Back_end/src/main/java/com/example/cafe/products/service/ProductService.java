package com.example.cafe.products.service;
...
import com.example.cafe.products.mapper.ProductMapper;
import com.example.cafe.products.repository.CategoryRepository;
import com.example.cafe.products.repository.ProductRepository;
import com.example.cafe.products.scraper.HighlandsCoffeeScraper;
import com.example.cafe.products.scraper.HighlandsProduct;
...
public class ProductService {
    ...
    public List<ProductResponse> importHighlandsCoffeeMenu() {
        ...
        try {
            ...
            throw new RuntimeException("Failed to scrape Highlands Coffee menu: " + e.getMessage());
        }

        for (HighlandsProduct p : scrapedProducts) {
            if (productRepository.existsByProductName(p.productName())) {
                continue;
            }

            Category category = categoryRepository.findByCategoryName(p.categoryName())
                    .orElseGet(() -> {
                        Category newCat = Category.builder().categoryName(p.categoryName()).build();
            ...
            Product product = Product.builder()
                    ...
                    .price(p.price())
                    .amount(100) // Default inventory
                    .category(category)
                    .build();

            if (p.imageUrl() != null && !p.imageUrl().isBlank()) {
                Image img = Image.builder().imageLink(p.imageUrl()).product(product).build();
